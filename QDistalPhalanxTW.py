...
dataset= Dataset('DISTALPHALANXtw', './datasets', 'DistalPhalanxTW_TRAIN.arff', 'DistalPhalanxTW_TEST.arff')
resultsFile= 'QDistalPhalanxTW_R.pkl' # None for not saving results data
dataset.shuffle()


#dataset.diff()
#dataset.TimeSeriesReshape( 128 )
print(dataset)


## Circuito cuántico variacional a usar
NUM_QUBITS= 6#dataset.requiredQubits()
...
MaxIter= 600
print('Unbalanced dataset. Setting class weights')
lossF= nn.CrossEntropyLoss(weight= torch.FloatTensor([1.5, 1.5, 0.5, 1.5, 2.0, 0.2]))
MaxExper= 30
accThreshold= 1.0#0.901
dataset.toTensor()
...
for exper in range(MaxExper):
    ...
    if exper >=len(allModels):
        ...
        model = torch.nn.Sequential(
            ...
            nn.Linear(dataset.TimeSeriesLength(), 100),
            nn.ReLU(),
            nn.Linear(100, 2**NUM_QUBITS),
            ...
            nn.Linear(2**NUM_QUBITS, dataset.numLabels()),nn.Softmax(dim=1)
        )
